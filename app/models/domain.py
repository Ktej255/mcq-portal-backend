...
class Subscription(Base):
    # NOTE: additive table. The migration must be autogenerated against the live
    # DB (`alembic revision --autogenerate -m "add subscriptions"`); the repo has
    # a multi-head/merge migration graph, so do not hand-write the down_revision
    # (run `alembic merge heads` first if needed).
    __tablename__ = "subscriptions"
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id"), index=True, nullable=False)
    tier = Column(Enum(SubscriptionTierEnum), default=SubscriptionTierEnum.FREE, nullable=False)
    billing_cycle = Column(Enum(BillingCycleEnum), default=BillingCycleEnum.MONTHLY, nullable=False)
    status = Column(Enum(SubscriptionStatusEnum), default=SubscriptionStatusEnum.ACTIVE, nullable=False)
    provider = Column(String, nullable=True)        # e.g. "cashfree"
    provider_ref = Column(String, nullable=True)    # order/subscription id from the provider
